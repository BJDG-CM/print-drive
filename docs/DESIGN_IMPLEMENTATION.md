...
- 신뢰 기기 전체 보관함, 공유 링크 안내, v2 public capability, 종료 결과를 분리한 진입 구조
- 기존 51개 DOM hook 보존과 추가 public/admin hook, runtime file row 연결
- 검색·filter·sort·선택 ZIP·preview·인쇄·암호화 update ZIP의 실제 state와 copy
...
- update ZIP은 `생성됨·미배포`로 표시하고 upload/deploy가 완료된 것처럼 보이지 않게 함
...
- browser 파일 추가는 encrypted ZIP download까지이며 저장소 write나 배포를 하지 않습니다.
